    ...
    try:
        ...
        try:
            if req.json()["status_code"] == 0:
                total_shares += 1
                print(Colorate.Horizontal(Colors.white_to_blue, f"[*] Shares Sent: {total_shares} [{raw_link}]"))
                pass
        except:
            fails += 1
...
if __name__ == "__main__":
    ...
    #shares input
    Write.Print("[?] Amount of shares [1 to ∞] ↓\n", Colors.blue_to_purple, interval=0.0001)
    amount = Write.Input(" >  ", Colors.blue_to_purple, interval=0.5, hide_cursor=True)
    #amount = (int(amount)*100)/100 #fill error gap

    #credits
    print(' ')
    print(' ')
    Write.Print("              * Thanks for using our Tool, Full credits to > Tekky#9999 x @auut | t.me/xtekky | .gg/onlp *\n", Colors.red_to_white, interval=0.01)
    print(' ')
    print(' ')
    ...
    def summary():
        ...
        if active_count() == count:
            if total_shares < amount:
                ...
                precision = 100 - (((total_shares - amount) / amount) * 100)
            Write.Print(f"[*] Status : COMPLETED | SPEED > {round((int(total_shares)/(time.time() - start_time - 4)), 2)}/s | TTC > {round(time.time() - start_time-3, 2)}s | AMOUNT > {total_shares} | PRECISION > {round(precision, 2)}%\n", Colors.red_to_white, interval=0.001)
            quit()
        else:
            sleep(2)
            if active_count() == 0:
                if total_shares < amount:
                    ...
                    precision = 100 - (((total_shares - amount) / amount) * 100)
                Write.Print(f"[*] Status : COMPLETED | SPEED > {round((int(total_shares)/(time.time() - start_time - 3)), 2)}/s | TTC > {round(time.time() - start_time-5, 2)}s | AMOUNT > {total_shares} | PRECISION > {round(precision, 2)}%\n", Colors.red_to_white, interval=0.001)
                quit()
            else:
                sleep(1)
                summary()
